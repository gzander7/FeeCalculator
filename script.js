...
function showAdvancedCalc() {
    ...
    if (showDetailsCheckbox.checked) {
        ...
        detailsElement.innerText += `\n${isNaN(ageMultiplier) || isNaN(length) || isNaN(width) ? '' : "$" + ageMultiplier.toFixed(2) + ' X ' + (length +4) + 'ft'  + ' X ' + (width + 4) + 'ft'} = $${isNaN(ageMultiplier) || isNaN(length) || isNaN(width) ? 0 : (ageMultiplier * (length + 4) * (width + 4)).toFixed(2)}`;
    } else {
